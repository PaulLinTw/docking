...
EPOCH = int(rds.get('LSTM_EPOCH'))        
BATCH_SIZE = int(rds.get('LSTM_BATCH'))   
LR = float(rds.get('LSTM_LR'))       
...
net = lstm_reg(train_x.shape[2]) 
data_start_time = time_range[0]
if rds.get('prediction_LSTM') is not None:
    para = pickle.loads(rds.get('prediction_LSTM'))
    net.load_state_dict(para)
    data_start_time = pickle.loads(rds.get('prediction_LSTM_start_time'))
...
for epoch in range(EPOCH):
    for step, (x, y) in enumerate(train_loader, 1):   # 分配 batch data, normalize x when iterate train_loader
        ...
        out = net(b_x)               # (mini_batch, 10, 1)
        loss = loss_func(out, b_y)     # MSE loss
...
rds.set('prediction_LSTM', pickle.dumps(net.state_dict()))
rds.set('prediction_vec_LSTM', pickle.dumps(vec))
rds.set('prediction_LSTM_start_time', pickle.dumps(data_start_time))
rds.set('prediction_LSTM_range', 'training data from [{}] to [{}]'.format(data_start_time, time_range[1]))

#########################################################################################################
######## 以下是為了正式使用 (採用所有至目前時間點的資料進行模型訓練) #################################
########################################### MLP_country #######################################################
df_feature = df_country >> select('action_type', 'product', 'country', 'counts')
vec_country = DictVectorizer(sparse=False)
data_feature = vec_country.fit_transform(df_feature.to_dict('records'))
counts_index = int(where(pd.Series(vec_country.get_feature_names()) == 'counts'))

df_x = df_country.iloc[:,4:4 + time_step] 
df_x = df_x[df_x.columns.sort_values(ascending=False)]
df_y = df_country.iloc[:,4 + time_step:]
data_x = df_x.as_matrix().reshape(df_x.shape[0], df_x.shape[1], 1)
data_y = df_y.as_matrix().reshape(df_y.shape[0], df_y.shape[1], 1)
data_feature = data_feature.reshape(data_feature.shape[0], 1, data_feature.shape[1])
data_feature = np.concatenate([data_feature for i in range(10)],axis = 1)
...
net = lstm_reg(train_x.shape[2])    
data_start_time = time_range[0]
if rds.get('prediction_LSTM_country') is not None:
    para = pickle.loads(rds.get('prediction_LSTM_country'))
    net.load_state_dict(para)
    data_start_time = pickle.loads(rds.get('prediction_LSTM_start_time'))
...
EPOCH = int(rds.get('LSTM_country_EPOCH'))        
BATCH_SIZE = int(rds.get('LSTM_country_BATCH'))   
LR = float(rds.get('LSTM_country_LR'))         
...
print(datetime.now(GMT), 'LSTM_country 訓練開始 !', file = open('log.txt', 'a'))
print(datetime.now(GMT), 'LSTM_country 訓練開始 !')
for epoch in range(EPOCH):
    for step, (x, y) in enumerate(train_loader, 1):   # 分配 batch data, normalize x when iterate train_loader
        b_x = Variable(x)   # batch x
        b_y = Variable(y)   # batch y
...
rds.set('prediction_LSTM_country', pickle.dumps(net.state_dict()))
rds.set('prediction_vec_country_LSTM', pickle.dumps(vec_country))
rds.set('prediction_LSTM_start_time', pickle.dumps(data_start_time))
rds.set('prediction_LSTM_range', 'training data from [{}] to [{}]'.format(data_start_time, time_range[1]))

print(datetime.now(GMT), 'create_model_LSTM 完成 !', file = open('log.txt', 'a'))
