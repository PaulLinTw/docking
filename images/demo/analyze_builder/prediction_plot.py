...
def read_data(product='a-tv', country='China', site = 'Beijing'):
    ...
    df = pd.DataFrame([i for i in query]) >> drop('_id')
    df_jo = pd.DataFrame([i for i in db.joins.find({'product':product,'country':country, 'site':site})]) >> drop('_id')
    df = pd.concat([df_jo  >> left_join(df[df.minute==i]) >> mutate(minute=i) for i in df.minute.unique()]).fillna(0).reset_index(drop = True)
...
def read_data_country(product='a-tv', country='China'):
    now = int((datetime.now(GMT)-init_time).total_seconds()/60)
    ...
    df = pd.DataFrame([i for i in query]) >> drop('_id')
    df_jo = pd.DataFrame([i for i in db.joins.find({'product':product,'country':country})]) >> drop('_id')
    df = pd.concat([df_jo  >> left_join(df[df.minute==i]) >> mutate(minute=i) for i in df.minute.unique()]).fillna(0).reset_index(drop = True)
...
def Forcast_country(df, k=10):
    rds = redis.Redis(host=incl.redis_servers, password=incl.redis_pass, port=incl.redis_port)
    Tree = pickle.loads(rds.get('prediction_tree_country'))
...
def draw2Dplotly_country(df_pre, output_type = 'file'):
    t_name = 'minute'
    conf = dict(scrollZoom = True,
                displaylogo= False,
                showLink = False,
...
def Forcast_and_Plot_Site(country_site = 'China_Beijing', product='a-tv'):
    df = read_data(country=country_site.split('_')[0], site = country_site.split('_')[1], product= product)
    div = []
    for action_type in df.action_type.unique():
        df_pre = Forcast(df >> mask(X.action_type==action_type))
        div.append(draw2Dplotly(df_pre, output_type = 'div'))
    return '<br>'.join(div)

def Forcast_and_Plot_Country(country = 'China', product='a-tv'):
    df = read_data_country(country=country, product= product)
    div = []
    for action_type in df.action_type.unique():
        df_pre = Forcast_country(df >> mask(X.action_type==action_type))
        div.append(draw2Dplotly_country(df_pre, output_type = 'div'))
    return '<br>'.join(div)
# df = read_data(product='a-tv', country='China', site = 'Beijing')
# df_pre = Forcast(df >> mask(X.action_type=='sell'))

# df = read_data_country(product='a-tv', country='China')
# df_pre = Forcast_country(df >> mask(X.action_type=='sell'))

# df_pre
# div = draw2Dplotly(df_pre)
# div = draw2Dplotly_country(df_pre)
# g=Forcast_and_Plot_Country()
