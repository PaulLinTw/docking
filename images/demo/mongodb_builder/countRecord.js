...
while (1)
{
	...
	if (m>counter)
	{
		...
		if (c==0)
		{
			c=db.records.find({ minute: counter }).count()
			if (c>0)
			{
				print("counting minute "+counter+" on records");
				db.records.aggregate( 
					{ $match: { minute : counter } }, 
					{ $group: { _id: {country:"$country",site:"$site",product:"$product",action:"$action"}, count: { $sum: 1 } } } 
				).forEach( function(x) {
