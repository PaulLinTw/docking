...
def read_data_country(product='a-tv', country='China', action = 'sell'):
    ...
    df >>= drop('_id')
    df_jo = pd.DataFrame([i for i in db.joins.find({'product':product,'country':country, 'action':action})]) 
    df_jo >>= drop('_id')
...
def Forcast_and_Plot_Country(country = 'China', product='a-tv', action = 'sell'):
    df = read_data_country(country=country, product= product, action= action)
    df_pre = Forcast_country(df)
    return draw2Dplotly_country(df_pre, output_type = 'div')
# df = read_data_country(product='a-tv', country='China', action = 'sell')
# df_pre = Forcast_country(df)

# df_preForcast_and_Plot_Country()
