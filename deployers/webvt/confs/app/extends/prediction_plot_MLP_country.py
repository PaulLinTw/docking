...
def load_model(): 
    ...
    mlp = MLP(len(vec.get_feature_names()))
...
def Forcast_country(df, model, k=10):
    ...
    for i in range(k):
        ...
        data_input = Variable(torch.FloatTensor(vec.transform(data)))
        p = mlp(data_input).data.numpy()[0, 0]    
        pre_value.append(p)
        t_value.append(df >> tail(1) >> r(X[t_name].astype(int).iloc[0] + i+1 ))
        current_feature.iloc[0] = p
    df_true = ddf.iloc[:,:5]   
    df_pre = df_true >> mutate(counts = mlp(Variable(torch.FloatTensor(vec.transform(ddf.to_dict('record'))))).data.numpy()[:,0]) 
